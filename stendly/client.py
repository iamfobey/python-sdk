...
class BaseClient:
    ...
    _ENVIRONMENTS = {
        ...
        "devnet": "https://devnet.api.stendly.com",
    }
    ...
    def __init__(
        ...
        Args:
            api_key: Merchant API key (starts with st_live_ or st_test_)
        ...
        Example:
            ...
            >>> api_key = "st_live_..."
            >>> if api_key.startswith("st_test_"):
            ...     env = "devnet"
            >>> elif api_key.startswith("st_live_"):
            ...     env = "mainnet"
            >>> else:
        ...
        Note:
            - API keys starting with "st_live_" must use mainnet
            - API keys starting with "st_test_" must use devnet
            - Mixing them will cause authentication errors
        """
        # Validate API key format
        if not (api_key.startswith("st_live_") or api_key.startswith("st_test_")):
            raise ValueError(
                "Invalid API key format. Must start with 'st_live_' (production) "
                "or 'st_test_' (development)."
            )
        
        # Auto-detect environment from API key if needed
        if api_key.startswith("st_test_") and environment == "mainnet":
            logger.warning(
                "Using test API key with mainnet environment. "
                "This will fail authentication. Use environment='devnet' for test keys."
            )
        elif api_key.startswith("st_live_") and environment == "devnet":
            logger.warning(
                "Using live API key with devnet environment. "
                "This will fail authentication. Use environment='mainnet' for live keys."
            )
...
class Client(BaseClient):
    ...
    def _init_namespaces(self) -> None:
        ...
        self.intents = IntentsNamespace(http_client=self._http_client)
        self.terminals = TerminalsNamespace(http_client=self._http_client)
        self.webhooks = WebhooksNamespace(http_client=self._http_client)
        self.merchant = MerchantNamespace(http_client=self._http_client)
...
class AsyncClient(BaseClient):
    ...
    def _init_namespaces(self) -> None:
        ...
        self.intents = AsyncIntentsNamespace(
            async_http_client=self._async_http_client,
        )
        self.terminals = AsyncTerminalsNamespace(
            async_http_client=self._async_http_client,
        )
        self.webhooks = AsyncWebhooksNamespace(
            async_http_client=self._async_http_client,
        )
        self.merchant = AsyncMerchantNamespace(
            async_http_client=self._async_http_client,
        )
